...
async def analyze_data(request: DataAnalysisRequest):
    ...
    analysis_prompt = f"""
        ...
        3. 如果有日期/時間欄位（例如：日期、年月、季度、timestamp等），且該欄位可以被解析為時間戳，通常作為 x 軸，並在回答中補充說明xAxis的type為datetime
        4. 如果 x 軸是分類資料（例如：產品名稱、地區、類別等）或無法轉換為時間戳的文字，則使用 categories 而非 datetime
        5. 如果有多個數值欄位，建議使用堆疊圖或分組圖
        6. 為圖表建議一個有意義的標題
        7. 建議適當的軸標題
